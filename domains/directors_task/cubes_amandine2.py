...
def robot_pick(agents, self_state, self_name, pickable):
...
def robot_place(agents, self_state, self_name, pickable, support):
...
def human_pick(agents, self_state, self_name, pickable):
    if self_name in self_state.isReachableBy[pickable] and self_state.isHolding[self_name] == []:
        for agent_name in agents:
            a = agents[agent_name]
            a.state.isReachableBy[pickable] = []
...
def human_place(agents, self_state, self_name, pickable, support):
    if self_state.hasOn[support] != []:
        return False
    if self_state.isOn[support] == []:
        return False
...
ctrl_operators = [robot_pick, robot_place, robot_wait]
unctrl_operators = [human_pick, human_place, human_place_stick, human_wait_for_cube]

cost_dict = {"robot_pick": 1, "robot_place": 1, "robot_wait": 1, "human_place_stick":1, "human_wait_for_cube":1,
             "human_pick": 1, "human_place": 1, "WAIT": 1, "IDLE": 0, "BEGIN": 0}

def robot_make_stack2(agents, self_state, self_name):
    return [("robot_pick", "red_cube_1"), ("robot_wait",), ("place_red_cube", "red_cube_1"), ("wait_stick",), ("robot_wait", ), ("place_blue_cube1", "blue_cube_1"),
...
def robot_place_red_cube(agents, self_state, self_name, cube):
    if self_state.isOn[cube] != []:
        return []
    return [[("robot_place", cube, "support1")], [("robot_place", cube, "support2")]]

def robot_wait_stick(agents, self_state, self_name):
    if self_state.isOn["cube_GGTB"] == []:
...
def robot_place_blue_cube1(agents, self_state, self_name, cube):
    ...
    for agent in agents:
        if agent == self_name:
            continue
        other_agent = agent
    if self_state.hasOn["cube_GGTB"] == [] and agents[other_agent].plan[-1].name == "human_wait_for_cube":
        return [("robot_pick", cube), ("robot_place", cube, "cube_GGTB"), ("robot_pick", "green_cube")]
    else:
        return [("robot_pick", "green_cube"), ("wait_blue_cube",)]

def robot_place_blue_cube2(agents, self_state, self_name, cube):
    if self_state.isOn[cube] != []:
        return []
    else:
        return [("robot_pick", cube), ("robot_place", cube, "green_cube")]

def robot_wait_blue_cube(agents, self_state, self_name):
    if same_last_tasks(agents[self_name].plan, 10, "robot_wait"):
        return False
    if self_state.hasOn["cube_GGTB"] != []:
        return []
    else:
...
def robot_place_green_cube(agents, self_state, self_name, cube):
    if self_state.isOn["blue_cube_1"] != []:
        return [("robot_place", cube, "blue_cube_1")]
    elif self_state.isOn["blue_cube_2"] != []:
        return [("robot_place", cube, "blue_cube_2")]
    return False
...
def human_make_stack(agents, self_state, self_name):
    return [("human_pick", "red_cube_2"), ("h_place_red_cube", "red_cube_2"), ("human_pick", "cube_GGTB"), ("human_place_stick", "cube_GGTB", "red_cube_1", "red_cube_2"),
            ("h_handle_blue_cube1", "blue_cube_2"), ("h_handle_blue_cube2", "blue_cube_2")]

@hatpehda.multi_decomposition
def human_place_red_cube(agents, self_state, self_name, cube):
    tasks = []
    if self_state.hasOn["support1"] == []:
        tasks.append([("human_place", cube, "support1")])
    if self_state.hasOn["support2"] == []:
        tasks.append([("human_place", cube, "support2")])
    return tasks
...
def human_handle_blue_cube_1_pick_place(agents, self_state, self_name, cube):
    return [("human_pick", cube), ("human_place", cube, "cube_GGTB")]
...
def human_handle_blue_cube_2(agents, self_state, self_name, cube):
    if self_state.isOn[cube] != []:
        ...
        return [("human_pick", cube), ("human_place", cube, "green_cube")]
...
def human_place_blue_cube(agents, self_state, self_name, cube):
    return [("human_place", cube, "cube_GGTB")]
...
unctrl_methods = [("h_place_red_cube", human_place_red_cube), ("h_handle_blue_cube1", human_handle_blue_cube_1_pick_place, human_handle_blue_cube_1_wait), ("h_make_stack", human_make_stack),
...
def on_plan_request(ctrl_agent_task, unctrl_agent_task):
    ...
    if len(unctrl_agent_task.keys()) != 1:
        ...
        return
    robot_name = list(ctrl_agent_task.keys())[0]
    human_name = list(unctrl_agent_task.keys())[0]
    state = hatpehda.State("robot_init")
    state.types = {"Agent": ["isHolding"], "Cube": ["isHeldBy", "isReachableBy"],
                   "Stick": ["isHeldBy", "isReachableBy"]}
    state_filled = state
    state_filled.individuals = {"Cube": ["red_cube_1", "red_cube_2", "blue_cube_1", "blue_cube_2", "green_cube"], "Stick": ["cube_GGTB"]}
    ...
    state_filled.isOn.update({"support1": ["table_1"], "support2": ["table_1"]})
    state_filled.hasOn = {m: [] for m in state_filled.individuals["Cube"] + state_filled.individuals["Stick"]}
    state_filled.hasOn.update({"support1": [], "support2": []})
    ...
    gui.show_plan(hatpehda.get_last_actions(root_action), robot_name, human_name)
